...
# Load locus data
df = pd.read_csv("locus_7_134368632.tsv", sep="\t")

# Load LD matrix
ld = pd.read_csv("locus_7_134368632.ld", sep="\t", header=None)
R_df = ld.values
...
# Save results
df.to_csv("finemapping_7_134368632.tsv", sep="\t", index=False)

# Save credible sets summary
with open("credible_sets_7_134368632.txt", "w") as f:
    if cs is not ro.NULL:
...
axes[0].legend()
axes[0].set_title(f"Fine-mapping results for CHR7:134368632")
...
plt.tight_layout()
plt.savefig("plot_7_134368632.png", dpi=300, bbox_inches='tight')
plt.close()
EOF
