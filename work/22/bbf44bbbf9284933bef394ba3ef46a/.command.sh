...
# Load locus data
df = pd.read_csv("locus_2_55513738.tsv", sep="\t")

# Load LD matrix
ld = pd.read_csv("locus_2_55513738.ld", sep="\t", header=None)
R_df = ld.values
...
# Save results
df.to_csv("finemapping_2_55513738.tsv", sep="\t", index=False)

# Save credible sets summary
with open("credible_sets_2_55513738.txt", "w") as f:
    if cs is not ro.NULL:
...
axes[0].legend()
axes[0].set_title(f"Fine-mapping results for CHR2:55513738")
...
plt.tight_layout()
plt.savefig("plot_2_55513738.png", dpi=300, bbox_inches='tight')
plt.close()
EOF
