...
# Load locus data
df = pd.read_csv("locus_20_42758834.tsv", sep="\t")

# Load LD matrix
ld = pd.read_csv("locus_20_42758834.ld", sep="\t", header=None)
R_df = ld.values
...
# Save results
df.to_csv("finemapping_20_42758834.tsv", sep="\t", index=False)

# Save credible sets summary
with open("credible_sets_20_42758834.txt", "w") as f:
    if cs is not ro.NULL:
...
axes[0].legend()
axes[0].set_title(f"Fine-mapping results for CHR20:42758834")
...
plt.tight_layout()
plt.savefig("plot_20_42758834.png", dpi=300, bbox_inches='tight')
plt.close()
EOF
