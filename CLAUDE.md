...
The `finemapping_susie_docker.nf` pipeline uses Docker containers instead of conda:
...
Config file: `nextflow.docker.config` (Docker enabled by default, `params.registry = "nextflow"`)

Run command:
```bash
nextflow run finemapping_susie_docker.nf \
  -c nextflow.docker.config \
  --sumstats /home/sagemaker-user/GWASTutorial/06_Association_tests/1kgeas.B1.glm.firth \
  --genotype_prefix /home/sagemaker-user/GWASTutorial/01_Dataset/1KG.EAS.auto.snp.norm.nodup.split.rare002.common015.missing \
  --outdir finemapping_results
```

## Input Data Locations (SageMaker)

- **Sumstats**: `/home/sagemaker-user/GWASTutorial/06_Association_tests/1kgeas.B1.glm.firth`
- **Genotype files**: `/home/sagemaker-user/GWASTutorial/01_Dataset/1KG.EAS.auto.snp.norm.nodup.split.rare002.common015.missing/`

Both are required inputs — genotype files are used as an LD reference panel in CALCULATE_LD, not generated by the pipeline.

### Transferring Data from SageMaker to Local Mac and Running Locally

**Step 1: Copy data from SageMaker to S3** (run on SageMaker)
```bash
aws s3 cp /home/sagemaker-user/GWASTutorial/06_Association_tests/1kgeas.B1.glm.firth s3://darp-dad-repos/RL/testdata/
aws s3 cp /home/sagemaker-user/GWASTutorial/01_Dataset/1KG.EAS.auto.snp.norm.nodup.split.rare002.common015.missing/ s3://darp-dad-repos/RL/testdata/genotypes/ --recursive
```

**Step 2: Download from S3 to Mac** (run on Mac)
```bash
aws s3 cp s3://darp-dad-repos/RL/testdata/ /Users/rli/Desktop/nextflow/testdata/ --recursive
```

**Step 3: Install Nextflow on Mac**
```bash
brew install nextflow
```

**Step 4: Run the Docker-based pipeline**
```bash
nextflow run finemapping_susie_docker.nf \
  -c nextflow.docker.config \
